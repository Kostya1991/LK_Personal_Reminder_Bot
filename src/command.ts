...
import {bot} from "./bot";
import {getList} from "./dataBaseHelper";
...
export function initCommand() {
    ...
    bot.command("list", async (ctx) => {
        const list = getList(ctx.chat.id.toString());
        await ctx.reply(`Календарь: \n${list.join("\n")}`, {parse_mode: "HTML"});
    });
}
